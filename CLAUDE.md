...
- `GET /announcement` (public), `PATCH /announcement` (admin-only, JSON body — `message`/`effect`/`active`) — the floating homepage announcement banner, managed from the admin panel's Content tab (above the carousel section). Single settings document (upserted in place, no list/IDs like carousel), collection `announcement`. `effect` is one of `balloons`/`sparkle`/`rain`/`confetti`/`snow`/`crackers`/`hearts`/`none`, validated against `VALID_ANNOUNCEMENT_EFFECTS` in `models.py`. `script.js`'s `loadAnnouncementBanner()` fetches this on page load, renders the matching particle animation into `#bdayEffectLayer` (`crackers` is the one special case — it renders as several radiating firework bursts via `renderCrackerBursts()`, not the generic per-particle loop the other effects share), and removes the overlay entirely if `active` is false or `message` is empty. Per-visitor dismissal is remembered in `localStorage` keyed by `updated_at + message` (not just the message, and not a boolean) — so any admin Save, even re-activating with identical wording, clears prior dismissals for everyone instead of staying hidden for people who saw it before.
- `POST /performances` and `PATCH /performances/{id}` (admin-only, JSON body — `title`/`description`/`location`/`youtube_url`/`category` [`EPA`|`PWP`], PATCH is a full replace via the same `PerformanceCreate` schema, not partial), `GET /performances` (public, all performances sorted newest-first), `DELETE /performances/{id}` (admin-only) — the homepage "Recent Performances" grid, managed from the admin panel's Content tab (below the carousel section). Only a raw `youtube_url` is accepted as input and never stored — `extract_youtube_video_id()` in `main.py` parses the actual video ID server-side (handles `watch?v=`, `youtu.be/`, `embed/`, `shorts/` links) and that's the only thing persisted, so the frontend only ever deals with a clean ID for the `youtube.com/embed/` iframe src. The admin edit form (`editPerformance()` in `admin.js`) reconstructs a canonical `youtube.com/watch?v={id}` URL to prefill that field, since the original submitted URL isn't stored. `script.js`'s `loadPerformances()` paginates client-side at 6 per page (mirrors the feedback wall's pagination widget/CSS classes) and, like the carousel, leaves the static fallback cards in `index.html` untouched if the API returns nothing.
